...
class CarbonFactor(models.Model):
    ...
    def _compute_recent_value(self):
        for factor in self:
            ...
            factor.recent_value_id = value_with_dates and max(value_with_dates, key=lambda f: f.date)
    ...
    def _compute_carbon_currency_id(self):
        for factor in self:
            factor.carbon_currency_id = self.env.ref("onsp_co2.carbon_kilo", raise_if_not_found=False) or self.env['res.currency']
    ...
    @api.depends('carbon_compute_method', 'value_ids.carbon_uom_id', 'value_ids.carbon_monetary_currency_id')
    def _compute_has_invalid_value(self):
        for factor in self:
            factor.has_invalid_value = (factor.carbon_compute_method == 'physical' and not all([v.carbon_uom_id for v in factor.value_ids])) or \
                                       (factor.carbon_compute_method == 'monetary' and not all([v.carbon_monetary_currency_id for v in factor.value_ids]))
    ...
    def _get_value_at_date(self, date=None):
        ...
        return self.value_ids and max(self.value_ids.filtered(lambda v: v.date <= date), key=lambda v: v.date)
