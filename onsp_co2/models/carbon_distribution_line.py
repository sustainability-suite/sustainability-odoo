...
class CarbonDistributionLine(models.Model):
    ...
    _sql_constraints = [
        ('positive_percentage', 'CHECK(percentage > 0)', 'Percentage must be higher than zero'),
        ('max_limit_percentage', 'CHECK(percentage <= 1)', 'Percentage cannot be higher than 100%'),
    ]
    ...
    res_model = fields.Char(related='res_model_id.model', index=True, precompute=True, store=True, readonly=True)
    ...
    carbon_type = fields.Selection([
        ('in', 'In'),
        ('out', 'Out'),
    ], string="Carbon Type", required=True)
    ...
    def create(self, vals_list):
        ...
        res = super(CarbonDistributionLine, self).create(vals_list)
        return res
