...
def create_carbon_factor(env, models: list[str] = None):
    for model in models:
        in_records = env[model].search([('carbon_in_is_manual', '=', True), ('carbon_in_factor_id', '=', False)])
        out_records = env[model].search([('carbon_out_is_manual', '=', True), ('carbon_out_factor_id', '=', False)])
...
def _create_carbon_factor(records, carbon_type):
    ...
    for rec in records:
        ...
        rec.write({
            f"{prefix}_factor_id": rec.env['carbon.factor'].create({
                ...
                'value_ids': [
                    (0, 0, {
                        ...
                        'carbon_monetary_currency_id': rec[f"{prefix}_monetary_currency_id"].id,
                    })
                ]
            }).id,
        })
