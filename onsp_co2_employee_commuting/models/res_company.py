...
class ResCompany(models.Model):
    ...
    employee_commuting_account_id = fields.Many2one('account.account', string="Employee Commuting Account", domain="[('deprecated', '=', False)]")
    ...
    def _cron_carbon_commuting_account_move_create(self):
        for company in self.env['res.company'].search([('employee_commuting_carbon_cronjob_active','=', True)]):
            ...
            last_account_move = self.env['account.move'].search([('company_id','=',company.id),('is_employee_commuting_carbon','=', True)], order="employee_commuting_carbon_date DESC", limit=1)
            last_account_move_date = datetime.combine(last_account_move.employee_commuting_carbon_date, time(1, 00)) if last_account_move else datetime.combine(company.carbon_lock_date , time(1, 00)) if company.carbon_lock_date else datetime.now().replace(day=1, month=1)

            # Check all months between last account_move and now
            date_set = rruleset()
            date_set.rrule(rrule(MONTHLY, bymonthday=(-1), dtstart=last_account_move_date, until=datetime.now()))
            date_set.exdate(last_account_move_date)
            for account_move_date in list(date_set):
                account_move_creation_success = company.carbon_commuting_create_account_move(account_move_date=account_move_date)
    ...
    def carbon_commuting_create_account_move(self, account_move_date):
        ...
        try:
            ...
            employees = (self.env['hr.employee'].search([('company_id','=',company.id)])._get_all_contracts(account_move_date, account_move_date, states=['open', 'close'])).employee_id
            if not employees:
                return False

            employees_with_commuting = employees.filtered('carbon_commuting_ids')
            decimal_precision = self.env['decimal.precision'].precision_get('Carbon value')
            ...
            for employee in employees - employees_with_commuting:
                if employee.km_home_work:
                    # 4 weeks per month
                    commuting_carbon = employee.km_home_work * 10 * average_carbon_per_km * WEEKS_PER_MONTH
            ...
            notification_message = f"Error processing employee {employee.name}: {str(e)}."
            _logger.info(notification_message)
            employee.message_post(body=notification_message, subject=f"Error Notification : Commuting Carbon print on the {account_move_date.strftime('%Y-%m-%d')}")
            return False
