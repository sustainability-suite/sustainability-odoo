...
def add_carbon_mode_columns(env):
    ...
    * ``sale.order.line`` carbon.line.mixin fields (the per-line values), and
    * the ``sale.order`` aggregates that sum those lines. Pre-creating only the
      lines is not enough: the order-level recompute re-reads every line anyway.

    See sustainability.hooks.add_fields for the rationale.
    """
    fields_spec = carbon_mixin_fields_spec(
        [("sale.order.line", "carbon.line.mixin")],
        "sustainability_sale",
    )
    fields_spec += [
        OpenupgradeField(
        ...
        OpenupgradeField(
            "carbon_currency_id", "sale.order", "many2one", "sustainability_sale"
        ),
    ]
