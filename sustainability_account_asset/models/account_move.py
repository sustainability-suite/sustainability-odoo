...
class AccountMove(models.Model):
    ...
    def _prepare_move_for_asset_depreciation(self, vals):
        ...
        for _dum, _my, line_vals in res["line_ids"]:
            carbon_value = (
                total_carbon_debt * line_vals["amount_currency"] / total_amount
            )
            sign = (
                -1 if asset.account_depreciation_id.id == line_vals["account_id"] else 1
            )
            line_vals.update(
                {"carbon_debt": carbon_value * sign, "carbon_is_locked": True}
            )
